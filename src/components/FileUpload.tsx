...
import { Upload, FileSpreadsheet, CheckCircle2, Server } from 'lucide-react';
        ...
        try {
            ...
            if (backendStatus === 'online') {
                try {
                    ...
                    if (!backendAnalysis.error && backendAnalysis.sample_data && backendAnalysis.sample_data.length > 0) {
                        data = backendAnalysis.sample_data;
                        console.log('✅ Backend analysis successful:', backendAnalysis);
                    } else {
                        throw new Error('Backend returned no data');
                    }
                } catch (error) {
                    console.warn('⚠️ Backend failed, using frontend parsing:', error);
                    backendAnalysis = null;
                }
            }

            // Fallback to frontend parsing if backend failed or unavailable
            if (!data || data.length === 0) {
                console.log('📊 Using frontend parsing...');
